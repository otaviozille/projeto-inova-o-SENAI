...
// PRG: Exibe mensagem se veio por GET
if (isset($_GET['success']) && $_GET['success'] === '1') {
  $mensagem = '<div class="alert alert-success mt-3">Comunidade cadastrada com sucesso!</div>';
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  $comunidade = trim($_POST['comunidade'] ?? '');
  $cidade     = trim($_POST['cidade'] ?? '');
  $estado     = trim($_POST['estado'] ?? '');
  $educacao   = trim($_POST['educacao'] ?? '');
  $agua       = trim($_POST['agua'] ?? '');
  $renda      = floatval(str_replace(',', '.', $_POST['renda'] ?? '0'));
  $saude      = trim($_POST['saude'] ?? '');
  $moradia    = trim($_POST['moradia'] ?? '');
  $emprego    = trim($_POST['emprego'] ?? '');

  if (
    $comunidade && $cidade && $estado && $educacao && $agua &&
    $renda > 0 && $saude && $moradia && $emprego
  ) {
    ...
    $mensagem = '<div class="alert alert-danger mt-3">Preencha todos os campos corretamente.</div>';
  }
}
?>
...
<body>
      ...
      <div class="container cabecalho-topo">
        ...
        <div class="botoes-header">
          ...
          <button class="btn-logout">
           ...
           <button class="btn-logout">
            <a style="text-decoration: none; color: white;" href="../painel-admin/painel.php">Voltar</a>
          </button>
        </div>
      </div>
      <!-- Título principal -->
  ...
  <script>
    ...
    // Remove o parâmetro ?success=1 da URL após exibir a mensagem
